...
# This script deploys using nginx reverse proxy to handle port 80
...
docker-compose -f docker-compose-port80.yml down 2>/dev/null || true
docker stop algodiscovery-frontend-prod 2>/dev/null || true
docker stop algodiscovery-nginx-proxy 2>/dev/null || true
docker rm algodiscovery-frontend-prod 2>/dev/null || true
docker rm algodiscovery-nginx-proxy 2>/dev/null || true
...
docker build --build-arg BUILD_ENV=prod:8080 -t algodiscovery-frontend .

# Start services in detached mode
echo "🚀 Starting production services with nginx reverse proxy..."
...
docker-compose -f docker-compose-port80.yml ps

# Check if containers are running
if docker ps | grep -q "algodiscovery-frontend-prod" && docker ps | grep -q "algodiscovery-nginx-proxy"; then
    echo ""
    ...
    echo ""
    echo "💡 Architecture:"
    echo "   Port 80 → Nginx Reverse Proxy → Frontend Container (Port 8080)"
    echo ""
else
    echo "❌ Deployment failed. Containers are not running."
    echo ""
    echo "🔍 Troubleshooting:"
    echo "   1. Check Docker logs: docker-compose -f docker-compose-port80.yml logs"
    echo "   2. Check container status: docker-compose -f docker-compose-port80.yml ps"
