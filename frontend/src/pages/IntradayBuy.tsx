...
import { IntradayRecommendation } from '../services/IntradayAPIService';
...
const IntradayBuy: React.FC = () => {
  const [recommendations, setRecommendations] = useState<IntradayRecommendation[]>([]);
  ...
  const fetchRecommendationsData = async (forceRefresh: boolean = false) => {
    try {
      ...
      console.log(`🎯 [${new Date().toISOString()}] IntradayBuy: Using new recommendation API service`);
      
      try {
        const recommendationResponse = await recommendationAPIService.getIntradayBuyRecommendations({
        ...
        if (recommendationResponse.status === 'success' && recommendationResponse.recommendations) {
          console.log(`✅ [${new Date().toISOString()}] IntradayBuy: Received ${recommendationResponse.recommendations.length} recommendations from new API`);
          
          // Transform the response to match our interface
          const transformedRecommendations: IntradayRecommendation[] = recommendationResponse.recommendations.map(rec => ({
            symbol: rec.symbol,
            companyName: rec.name,
            score: rec.score,
            currentPrice: rec.price ?? 0,
            target: (rec.price ?? 0) * 1.05, // Default 5% target for intraday
            stopLoss: (rec.price ?? 0) * 0.98, // Default 2% stop loss for intraday
          ...
          console.warn(`⚠️ [${new Date().toISOString()}] IntradayBuy: New API returned no recommendations, falling back to theme recommendations`);
        }
      } catch (newApiError) {
        console.warn(`⚠️ [${new Date().toISOString()}] IntradayBuy: New recommendation API failed, falling back to theme recommendations:`, newApiError);
      }

      // Use theme recommendations if enabled
      if (useThemeRecommendations) {
        console.log(`🎯 [${new Date().toISOString()}] IntradayBuy: Using theme recommendations`);
        ...
        if (themeResponse.success && themeResponse.recommendations) {
          console.log(`✅ [${new Date().toISOString()}] IntradayBuy: Received ${themeResponse.recommendations.length} theme recommendations`);
          setRecommendations(themeResponse.recommendations as IntradayRecommendation[]);
          return;
        } else {
          console.error(`❌ [${new Date().toISOString()}] IntradayBuy: Theme recommendations failed:`, themeResponse.error);
        }
      }

      // Regular recommendations fallback via centralized service
      console.log(`🔄 [${new Date().toISOString()}] IntradayBuy: Fallback to theme recommendation endpoint`);
      const fallback = await recommendationAPIService.getIntradayBuyRecommendations({
        max_recommendations: 50,
        min_score: 60.0,
        risk_profile: 'aggressive',
        include_metadata: true
      ...
      if (fallback.status === 'success' && fallback.recommendations) {
        const transformed = fallback.recommendations.map(rec => ({
          symbol: rec.symbol,
          companyName: rec.name,
          score: rec.score,
          currentPrice: rec.price ?? 0,
          target: (rec.price ?? 0) * 1.05,
          stopLoss: (rec.price ?? 0) * 0.98,
          change: (rec.price ?? 0) * (rec.change_percent / 100),
          changePercent: rec.change_percent,
          volume: rec.volume,
          sector: rec.sector,
        ...
        setRecommendations(transformed as IntradayRecommendation[]);
      } else {
        throw new Error(fallback.error || 'Fallback recommendation call failed');
      }
    } catch (err: any) {
      console.error(`❌ [${new Date().toISOString()}] IntradayBuy: Failed to fetch recommendations:`, {
        error: err.message,
        forceRefresh,
        aiMode,
  ...
  const {
    ...
    timeout: 30000,
    enableCaching: true,
    strategy: aiMode ? 'intraday-buy-ai' : 'intraday-buy',
    ...
    <Box sx={{ p: 3 }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
            ...
            <Box>
              ...
              <Typography variant="subtitle1" color="text.secondary">
                AI-powered intraday trading with real-time market analysis
              </Typography>
            </Box>
          </Box>
            ...
            <TabPanel value={advancedTabValue} index={1}>
              <Box sx={{ p: 2, bgcolor: 'background.paper', borderRadius: 1, border: '1px solid', borderColor: 'divider' }}>
                <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
                  Test intraday-related queries and algorithms
                </Typography>
                <Grid container spacing={2}>
                  <Grid item xs={12}>
                    <FormControl fullWidth>
                      ...
                      <Select
                        ...
                        label="Query"
                        placeholder="Enter your intraday trading query..."
                      >
                        <MenuItem value="momentum_intraday">Momentum Intraday Strategy</MenuItem>
                        <MenuItem value="breakout_intraday">Breakout Intraday Strategy</MenuItem>
                        <MenuItem value="volume_surge">Volume Surge Strategy</MenuItem>
      ...
      {loading && (
        <Box sx={{ mb: 3 }}>
          ...
          <Typography variant="body2" sx={{ mt: 1, textAlign: 'center' }}>
            AI is analyzing intraday patterns...
          </Typography>
        </Box>
      )}
      ...
      <Alert severity="info" sx={{ mb: 3 }}>
        <Typography variant="body2">
          <strong>Intraday Strategy:</strong> These recommendations are designed for same-day trading with entry between 9:30 AM - 2:00 PM and mandatory exit by 3:20 PM. 
          Focus on stocks with volume surge, gap-up patterns, and strong momentum indicators.
        </Typography>
      </Alert>
      ...
      <Card>
        <CardContent>
          <Typography variant="h6" sx={{ mb: 2, display: 'flex', alignItems: 'center', gap: 1 }}>
            <AutoAwesome />
            AI-Powered Intraday Buy Recommendations
            <Chip 
              ...
              <Table stickyHeader>
                <TableHead>
                  <TableRow>
                    ...
                    <TableCell><strong>Gap Up</strong></TableCell>
                    ...
                    >
                      ...
                      <TableCell>
                        <Chip
                          ...
                          color={stock.volumeSurge >= 2 ? 'success' : 'warning'}
                          size="small"
                        />
                      </TableCell>
                      <TableCell>
                        <Typography variant="body2" color={stock.gapUp > 1 ? 'success.main' : 'text.secondary'}>
                          {(stock.gapUp || 0).toFixed(1)}%
                        </Typography>
                      </TableCell>
                      <TableCell>
                        <Chip
