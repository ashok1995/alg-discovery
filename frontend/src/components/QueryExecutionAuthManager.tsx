/**
 * Query execution authentication (VNC session + session-status).
 * Proxies: /api/chartink, /api/chartink-health (backend unchanged).
 */
...
const QueryExecutionAuthManager: React.FC = () => {
  const {
    sessionStatus,
    loading,
    error,
    ...
    <Card>
      <CardContent>
        <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
          <Security sx={{ mr: 1 }} />
          <Typography variant="h6">Query execution authentication</Typography>
          <Box sx={{ flexGrow: 1 }} />
          ...
          <>
            ...
            {isAuthenticated && (
              <Alert severity="success" sx={{ mb: 2 }}>
                Query execution is authenticated and ready to fetch stock data.
              </Alert>
            )}
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
              ...
              <Alert severity="info" variant="outlined" sx={{ py: 0.5 }}>
                <Typography variant="caption" display="block">
                  Flow: 1) Open VNC. 2) Log in to the provider in the VNC session. 3) Force Authenticate. 4) Wait 1 min, then Check Status.
                </Typography>
              </Alert>
        ...
        {!sessionStatus && !loading && (
          <Alert severity="info">Click refresh to check query execution authentication status.</Alert>
        )}
...
export default QueryExecutionAuthManager;
