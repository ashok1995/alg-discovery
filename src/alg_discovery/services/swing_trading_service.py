...
from api.services.intraday_service import ChartinkIntegration
...
class SwingTradingAnalyzer:
    ...
    def __init__(self, data_service: RealTimeDataService):
        ...
        if config_manager.last_loaded is None:
            import asyncio
            # Run the async initialization synchronously in constructor
            try:
                loop = asyncio.get_event_loop()
                if loop.is_running():
                    # If we're already in an async context, we can't use run_until_complete
                    # Schedule initialization for later
                    logger.warning("⚠️ Config manager needs initialization - will initialize on first use")
                else:
                    loop.run_until_complete(config_manager.initialize())
                    logger.info("🔧 Config manager initialized in SwingTradingAnalyzer")
            except RuntimeError:
...
class SwingTradingService:
    ...
    async def get_swing_buy_recommendations(self, limit: int = 10,
        ...
        try:
            # Get candidates using analyzer's configuration-driven approach
            candidates = await self.analyzer.get_candidates_from_config("swing_buy")
            
            recommendations = []
            for symbol in candidates[:limit * 2]:  # Get more candidates to filter
                # Analyze each stock with config
                analysis = await self.analyzer.analyze_stock_with_config(symbol, "swing_buy")
                if not analysis:
                    continue
                    
                # Apply confidence and strength thresholds
                if (analysis['confidence'] >= confidence_threshold and 
                    analysis['strength'] >= strength_threshold):
                    
                    # Create IntradaySignal object
                    signal = IntradaySignal(
                        ...
                        volume_spike=analysis['indicators'].get('volume_ratio', 1.0) > 1.5,
                        breakout_type="swing_momentum",
                        risk_reward_ratio=analysis['target_return_pct'] / analysis['stop_loss_pct']
                    )
    ...
    async def get_short_term_buy_recommendations(self, limit: int = 10,
        ...
        try:
            # Get candidates using analyzer's configuration-driven approach
            candidates = await self.analyzer.get_candidates_from_config("short_term_buy")
            
            recommendations = []
            for symbol in candidates[:limit * 2]:  # Get more candidates to filter
                # Analyze each stock with config
                analysis = await self.analyzer.analyze_stock_with_config(symbol, "short_term_buy")
                if not analysis:
                    continue
                ...
                if (analysis['confidence'] >= confidence_threshold and 
                    ...
                    signal = IntradaySignal(
                        ...
                        indicators=analysis['indicators'],
                        volume_spike=analysis['indicators'].get('volume_ratio', 1.0) > 1.5,
                        breakout_type="short_term_momentum",
                        risk_reward_ratio=analysis['target_return_pct'] / analysis['stop_loss_pct']
                    )
                    recommendations.append(signal)
    ...
    async def get_long_term_buy_recommendations(self, limit: int = 10,
        ...
        try:
            # Get candidates using analyzer's configuration-driven approach
            candidates = await self.analyzer.get_candidates_from_config("long_term_buy")
            
            recommendations = []
            for symbol in candidates[:limit * 2]:  # Get more candidates to filter
                # Analyze each stock with config
                analysis = await self.analyzer.analyze_stock_with_config(symbol, "long_term_buy")
                if not analysis:
                    continue
