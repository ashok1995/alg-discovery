...
class ChartinkIntegration:
    ...
    def __init__(self):
        self.config_path = (
            files("alg_discovery.recommendation.config") / "chartink_filters.json"
        ).as_posix()
        self.filters_config = {}
        self.session = None
...
class IntradayService:
    ...
    async def health_check(self) -> Dict[str, Any]:
        """
        Perform comprehensive health check of the intraday service.
        ...
        try:
            ...
            if self.data_service:
                ...
                try:
                    stock_data = await self.data_service.get_stock_data(test_symbol)
                    health_status["components"]["data_service"] = "healthy" if stock_data else "degraded"
                except Exception as e:
                    health_status["components"]["data_service"] = f"error: {str(e)[:50]}"
            
            # Test ChartInk integration
            try:
                chartink = ChartinkIntegration()
                stocks = await chartink.get_stocks_from_chartink("intraday_buy", limit=5)
                health_status["components"]["chartink_integration"] = "healthy" if stocks else "degraded"
                health_status["metrics"]["total_symbols_tracked"] = len(stocks)
            except Exception as e:
                health_status["components"]["chartink_integration"] = f"error: {str(e)[:50]}"
    ...
    async def get_available_variants(self) -> Dict[str, Any]:
        ...
        Returns:
            Dict containing available categories and their variants
        """
        try:
            # Load the intraday buy configuration
            config = load_config("intraday_buy")
            ...
            for category, versions in sub_algorithms.items():
                variants_list = []
                for version, details in versions.items():
                    variants_list.append({
                        "version": version,
                        "name": details.get("name", f"{category} {version}"),
